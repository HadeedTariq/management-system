...
const PostCard = ({ post }: { post: SocietyDetailsResponse["posts"][0] }) => (
  <Card className="group overflow-hidden border-slate-200 shadow-sm transition-shadow hover:shadow-md">
    ...
    <div className="relative h-40 w-full overflow-hidden bg-gradient-to-br from-slate-100 to-slate-50">
      ...
      <div className="absolute right-2.5 top-2.5">
        {post.isPublished ? (
          <Badge className="bg-white/90 text-emerald-700 border border-emerald-200 text-[10px] font-semibold backdrop-blur-sm hover:bg-white/90">
            Published
          </Badge>
        ) : (
          <Badge className="bg-white/90 text-slate-500 border border-slate-200 text-[10px] font-semibold backdrop-blur-sm hover:bg-white/90">
            Draft
          </Badge>
        )}
      </div>
...
const EventRow = ({
  ...
  >
    ...
    <div className="min-w-0 flex-1 space-y-1.5">
      ...
      <div className="flex flex-wrap items-center gap-4 pt-0.5 text-[11px] text-slate-400">
        <span className="flex items-center gap-1.5">
